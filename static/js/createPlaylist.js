window.addEventListener("load", () => {
	...
	const createPlaylistToggle = document.getElementById("createPlaylistToggle");
	const create_playlist = document.getElementsByClassName("create-playlist")[0];
	...
	createPlaylistToggle.addEventListener("click", () => {
		create_playlist.toggleAttribute("hidden");
	});
	...
	createPlaylist.addEventListener("click", () => {
		fetch("/playlists", {
			...
			body: JSON.stringify({
				...
				games: Array.from(document.querySelectorAll("#listSelected>li>input")).map((element) => element.value)
			}),
		})
		.then(async (response) => {
			if(!response.ok) {
